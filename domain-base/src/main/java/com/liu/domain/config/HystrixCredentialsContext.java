...
/**
 * 现在遇到的问题是，从一个 Service 向另外一个 Service 转发的时候，Credentials 丢失，
 * 原因是 Hystrix 不转发；所以，需要补发，该类的逻辑就是取转发相关遗漏的 Credentials；在 OAuth 认证中是关键；
 * @author zyw
 * @date 2018/2/6
 */
@Configuration
public class HystrixCredentialsContext {
    ...
    private static HystrixRequestVariableDefault<Authentication> getInstance() {
        ...
        return requestTemplate -> {
            ...
            if( auth != null ){
                ...
                if(details instanceof OAuth2AuthenticationDetails) {
                    ...
                    requestTemplate.header(HttpHeaders.AUTHORIZATION, BEARER_TOKEN_TYPE + ( (OAuth2AuthenticationDetails) details).getTokenValue() );
                }
            }else{
